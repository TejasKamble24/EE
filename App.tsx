...
const App: React.FC = () => {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-[2rem] p-10 shadow-2xl max-w-sm w-full mx-6 transform transition-all scale-100">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-2">Enter PIN to continue to {pinTarget === 'home' ? 'Teacher Platform' : 'Business Plan'}.</p>
        </div>
        ...
        <button 
          ...
          className="w-full mt-6 text-emerald-600 font-bold hover:text-emerald-700 transition flex items-center justify-center gap-2 border-t pt-6"
        >
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {MOCK_COURSES.map(course => (
          <div key={course.id} onClick={() => handleCourseClick(course)} className="bg-white rounded-2xl border border-slate-100 overflow-hidden hover:shadow-xl transition group cursor-pointer">
            ...
            <div className="flex flex-col md:flex-row gap-12">
               <div className="flex-[2]">
                  ...
                  <div className="flex flex-wrap gap-6 text-sm font-bold uppercase tracking-widest text-slate-400">
                    ...
                    <div>{selectedCourse.price === 'Free' ? 'Free' : `$${selectedCourse.price}`}</div>
                  </div>
               </div>
               <div className="flex-1">
                 <div className="bg-white rounded-2xl p-6 text-slate-900 shadow-2xl">
                    ...
                    <button onClick={() => startLearning(selectedCourse)} className="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold hover:bg-emerald-700 transition shadow-lg">Enroll Now</button>
                 </div>
               </div>
            </div>
          </div>
    ...
    <div className="min-h-[80vh] flex flex-col bg-slate-50">
      ...
      <div className="flex-1 flex items-center justify-center px-6">
        <div className="max-w-md w-full bg-white p-10 rounded-3xl shadow-xl border border-slate-100">
          ...
          <button onClick={handleLogin} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-bold hover:bg-emerald-700 transition">Continue to App</button>
        </div>
      </div>
    </div>
  );
  ...
  const renderDashboard = () => (
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-6 flex flex-col md:flex-row gap-8 items-center shadow-sm">
         <img src="https://picsum.photos/seed/c1/300/200" className="w-full md:w-48 h-32 rounded-xl object-cover" alt="Course" />
         <div className="flex-1">
            <h3 className="text-xl font-bold mb-2">Teach with Tech</h3>
            <button onClick={() => startLearning(TEACH_WITH_TECH_COURSE)} className="bg-emerald-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-emerald-700 transition">Resume Course</button>
         </div>
      </div>
    </div>
  );
  ...
  const renderPlayer = () => {
    ...
    return (
      <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden">
        <GlobalBackButton />
        <div className="flex flex-col md:flex-row flex-1 overflow-hidden mt-6">
            ...
            <div className="w-full md:w-80 bg-white border-l border-slate-100 overflow-y-auto p-4">
                <h2 className="font-bold text-lg mb-4">{selectedCourse.title}</h2>
                {selectedCourse.modules.map(mod => (
                  <div key={mod.id} className="mb-4">
                    <p className="text-xs font-bold text-slate-400 mb-2">{mod.title}</p>
                    {mod.lessons.map(les => (
                      <div key={les.id} onClick={() => setActiveLesson(les)} className={`p-2 rounded-lg cursor-pointer ${activeLesson.id === les.id ? 'bg-emerald-50 text-emerald-700 font-bold' : ''}`}>
                        {les.title}
                      </div>
                    ))}
                  </div>
      ...
      <div className="mt-8">
        ...
        <div className="space-y-6 mt-8">
           {MOCK_DISCUSSIONS.map(disc => (
             <div key={disc.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                <h3 className="text-xl font-bold mb-2">{disc.title}</h3>
                <p className="text-slate-600">{disc.content}</p>
             </div>
           ))}
        </div>
      </div>
  ...
  const renderProfile = () => (
    <div className="max-w-4xl mx-auto px-6 py-12">
      <GlobalBackButton />
      <div className="bg-white rounded-3xl border border-slate-100 p-10 mt-6 text-center">
         <img src={user?.avatar} className="w-24 h-24 rounded-full mx-auto mb-4 border-4 border-emerald-500" alt="User" />
  ...
  const renderContact = () => (
    <div className="min-h-[70vh] flex flex-col items-center justify-center px-6">
      ...
      <div className="mt-20 text-center animate-in fade-in zoom-in duration-500">
        ...
        <div className="text-3xl font-extrabold text-emerald-600 bg-emerald-50 px-8 py-4 rounded-2xl border border-emerald-100 shadow-sm inline-block">Connect TSK</div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="transition-all duration-300">
        ...
        {currentPage === 'business-plan' && <div className="py-12"><GlobalBackButton /><BusinessPlan /></div>}
      ...
      <footer className="bg-white border-t border-slate-100 py-12">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
           ...
           <div className="flex gap-8 text-sm text-slate-500">
              <button onClick={() => setCurrentPage('contact')} className="hover:text-emerald-600 transition font-medium">Contact</button>
           </div>
